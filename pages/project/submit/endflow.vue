...
const openWorkFlow = reactive({
  ...
  score: 93, // score=-1表示没有接受
  records: [
    {
      time: '2022-12-12-22:00',
      type: 1, // 1表示学生提交，2表示老师回复
      fileNames: ["第一版结题报告.doc", "答辩PPT.ppt", "使用指南.pdf",],
      fileUrls: ["url1", "url2", "url3"],
      msg: "提交了本项目的答辩文档以及ppt,并且对于项目开源代码写了一份部署指南。"
    }, {
      time: '2022-12-12-22:13',
      type: 2,
      advices: ["需要进一步明确一下最终使用场景", "测试部分流程不够清晰"]
    }, {
      time: '2022-12-12-22:35',
      type: 1, // 1表示学生提交，2表示老师回复
      fileNames: ["第一版开题报告.doc"],
      fileUrls: ["url1"],
      msg: "已按照老师要求完成修改。"
    }, {
      time: '2022-12-12-23:00',
      type: 2,
      advices: ["逻辑清晰，工作量充足"]
    }
  ]
...
const upSubmit = () => {
  ...
  router.push(`/project/submit/create?id=${route.query.id}&flow=1`);
}
const upReply = () => {
  // flow=0 表示开题， flow=1表示结题
  router.push(`/project/submit/reply?id=${route.query.id}&flow=1`);
}
const getFile = (url) => {
  // TODO:
  // 点击下载链接
